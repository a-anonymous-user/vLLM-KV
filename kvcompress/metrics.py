...
class CompressionMetrics:
    ...
    def schedule_evictions(
        ...
        if PyramidKV:
            for i in range(self.kv_metric_head_bias.bias.size(0)):  # bias.size(0) 
                self.kv_metric_head_bias.bias[i, :, :] = i + 1
        ...
        else:  # Uniform evict first attempt
